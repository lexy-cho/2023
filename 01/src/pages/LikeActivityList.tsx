...
function LikeActivityList() {
    ...
    const getLikeActivityList = async (): Promise<any> => {
        const params = { actType: actType.id, actStat: actStat.id, memUid: user?.memUid };
        try {
            const response: any = await activityApi.getLikeActivityList(params);
    ...
    const { data, isLoading, isError, refetch } = useQuery<{ cnt: Number, list: any }>(["getLikeActivityList", actType, actStat], getLikeActivityList, { enabled: !!userInfo, cacheTime: 0, suspense:false }); 
    ...
    useEffect(() => {
        if (!userInfo?.memberNormalVO) setVisible(true);
    }, [])

    return (
        <>
            <DefaultHeader title={'찜한활동'} headType={'sub'} />
            <div id="container">
                <section>
                    <div className="contents">
                        ...
                        <ul className="list">
                            ...
                            {
                                (actList?.length === 0) && <li>찜한 활동이 없습니다.</li>
                            }
                            {/* end Loop */}
                        </ul>
                ...
                {popUpVisible &&
                    <div className="popDim">
                        <div className="basicPop">
                            ...
                            <div className="pBtn">
                                <button className="btnG" onClick={() => handleType()}>확인</button>
                            </div>
                        </div>
                    </div>
                }
...
export default LikeActivityList;
