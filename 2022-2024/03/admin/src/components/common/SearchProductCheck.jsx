...
const SearchProductCheck = forwardRef(({ title, list, onCheck, isCheckSync = false, checked, isAll, isChipCase, onChangeCheck = () => {}, isSearch, readOnly, isHeader, isFilter = false }, ref) => {
  ...
  return (
    <div className={`multiSSet ${isChipCase ? 'chipCase' : ''} ${open ? 'open' : ''}`} ref={ref}>
      {isChipCase ? (
        ...
        <div className="item">
          {isAll && (
            <span className="checkSet">
              <input checked={selectCheck?.length === data?.length} type="checkbox" id={'ALL'} onChange={(e) => handleAllCheck(e.target.checked)} />
...
SearchProductCheck.defaultProps = {
  isChipCase: false,
  isSearch: true,
  isHeader: false,
  readOnly: false,
};
export default SearchProductCheck;
