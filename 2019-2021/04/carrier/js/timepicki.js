...
(function($) {
	...
	$.fn.timepicki = function(options) {
		...
		var defaults = {
			format_output: function(tim, mini, meri) {
			    if (settings.show_meridian) {
					...
					// return tim + ":" + mini + " " + meri; Lexy
					return meri + " " + tim + ":" + mini;
			    } else {
		...
		return this.each(function() {
				...
				"<div class='timepicker_wrap " + settings.custom_classes + "'>" +
					"<div class='timepicker_back'>" +
						...
						"<div class='mins'>" +
							...
							bottom_arrow_button +
						"</div>" +
					"</div><div class='timePickerBack'></div>");
			if(settings.show_meridian){
				new_ele.find('.timepicker_back').prepend( // Lexy 위치변경
					"<div class='meridian'>" +
						top_arrow_button +
						"<div class='mer_tx'><input type='text' class='timepicki-input' disabled></div>" +
						bottom_arrow_button +
					"</div>");
			}
			if(settings.reset){
				new_ele.find('.timepicker_back').append(
					"<div class='btnArea tC'><button class='btnS reset_time'>초기화</button><button class='btnS timerClose'>적용</button></div>");
			}
			ele_par.append(new_ele);
			//$(new_ele).wrap("<div class='wrap'></div>");
			...
			$('.timerClose').on("click", function(event) {
				close_timepicki();
			});
			...
			function open_timepicki() {
				...
				ele_next.fadeIn();
				$(".timePickerBack").addClass('on'); //lexy
				$('body').css({'overflow':'hidden'}); //lexy
			...
			function close_timepicki() {
				ele_next.fadeOut();
				$(".timePickerBack").removeClass('on'); //lexy
				$('body').css({'overflow':'initial'}); //lexy
			}
			...
			function change_meri(cur_ele, direction) {
				...
				if ((cur_ele && cur_ele.hasClass('action-next')) || direction === 'next') {
					if (cur_mer == "오전") {
						ele_next.find("." + cur_cli + " .mer_tx input").val("오후");
					} else {
						ele_next.find("." + cur_cli + " .mer_tx input").val("오전");
					}
				} else if ((cur_ele && cur_ele.hasClass('action-prev')) || direction === 'prev') {
					if (cur_mer == "오전") {
						ele_next.find("." + cur_cli + " .mer_tx input").val("오후");
